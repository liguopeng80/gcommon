...
class WsJsonClientProtocol(WebSocketClientProtocol):
    ...
    def onOpen(self):
        logger.info(
            "[%06x], connected to server (WS). total clients: %s.",
            self.client_id,
    ...
    def onConnect(self, response):
        logger.info(
            "[%06x], connecting to server (WS). total clients: %s.",
            self.client_id,
            self._active_clients,
        )
    ...
    def onClose(self, was_clean, code, reason):
        ...
        logger.info(
            "[%06x], connection lost (WS). total clients: %s.",
            self.client_id,
            self._active_clients,
        )
    ...
    def connectionMade(self):
        ...
        logger.info(
            "[%06x], connected to server (TCP). total clients: %s.",
            self.client_id,
            self._active_clients,
        )
